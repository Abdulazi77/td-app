...
# Classic simple view with optional “T&D Model Chart” safety overlays.
#
# Methods:
...
# References: Johancsik SPE-11380; API RP 7G addendum; minimum curvature technical notes.
#
# Notes:
# This file is self-contained. Keep requirements.txt minimal:
#   streamlit
#   plotly
#   pandas
#   numpy
...
def soft_string_stepper(md, inc_deg, kappa_rad_per_ft, cased_mask,
                        comp_along_depth, comp_props,  # 'DC'/'HWDP'/'DP' per segment
    ...
    # --- per-segment curvature (kappa_seg) ---
    kappa_arr = np.asarray(kappa_rad_per_ft)
    if kappa_arr.shape[0] == md.shape[0]:
        kappa_seg = kappa_arr[:-1]       # station → segment
    else:
        kappa_seg = kappa_arr            # already per-segment

    nseg = md.shape[0] - 1
    if nseg <= 0:
        raise ValueError("Need at least two MD stations for T&D.")

    # per-segment inclination (lower station)
    ...
    # cased/open per segment
    cased_seg = np.asarray(cased_mask)[:nseg]

    # fill component/friction per segment
    for i in range(nseg):
...
with tab1:
    ...
    fig3d = go.Figure(data=[go.Scatter3d(
        x=E, y=N, z=-TVD, mode="lines", line=dict(width=6, color="#4cc9f0"), name="Wellpath"
    )])
...
with tab2:
    ...
    if not simple_mode:
        ...
        # μ-sweep helper (depth & torque returned with matching lengths)
