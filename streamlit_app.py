...
Left plot unchanged (old style). Right plot updated.
"""
...
with tab:
    ...
    if not simple_mode:
        ...
        # ---------- LEFT PLOT (REVERTED: old behavior) ----------
        fig_left = go.Figure()
        for mu in mu_band:
        ...
        # NOTE: Top-drive limit intentionally NOT shown on the left (old spec)
        ...
        # ---------- RIGHT PLOT (UPDATED ONLY) ----------
        fig_right = go.Figure()
        for mu in mu_band:
            dmu, tmu = run_td_off_bottom(mu)
            # ensure highest at surface for elemental curve
            if tmu[0] < tmu[-1]:
                tmu = tmu[::-1]
        ...
        # Combined-load envelope (pickup tension)
