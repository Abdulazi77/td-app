...
Single-tab app: survey → casing/open-hole → T&D. Dr.-style plots.
"""
...
# Plotly client-side download config (camera icon)
PLOT_CONFIG = {
    "displaylogo": False,
...
def png_download_button(fig: go.Figure, label: str, filename: str):
    """Server-side PNG (optional). Safe if Kaleido is missing."""
    try:
        import plotly.io as pio  # requires kaleido on server
        buf: bytes = pio.to_image(fig, format="png", scale=2)
        st.download_button(label, data=buf, file_name=filename, mime="image/png")
    except Exception:
        # Why: kaleido often not installed on PaaS; avoid crashing the app.
        st.caption("Tip: use the camera icon on the chart to download PNG (no Kaleido needed).")

# Minimal casing & tool-joint sets
...
# ----------------------------------------------------------------------
# Combined-load surrogate (API 7G-like)
# ----------------------------------------------------------------------
...
with tab:
    ...
    st.plotly_chart(fig2d, use_container_width=True, config=PLOT_CONFIG)
    ...
    # LEFT (risk curves; keep original behavior)
    fig_left = go.Figure()
    for mu in mu_band:
    ...
    # RIGHT (elemental torque + limits + combined-load)
    fig_right = go.Figure()
    for mu in mu_band:
        dmu, tmu = run_td_off_bottom(mu)
        # ensure highest at surface (decreasing with depth)
        if tmu[0] < tmu[-1]:
            tmu = tmu[::-1]
    ...
    fig_right.add_trace(go.Scatter(x=T_allow/1000.0, y=df_pick["md_bot_ft"].to_numpy(),
                                   mode="lines", name="TJ combined-load limit", line=dict(dash="dot")))
    ...
    cL, cR = st.columns(2)
    with cL:
        st.plotly_chart(fig_left, use_container_width=True, config=PLOT_CONFIG)
        png_download_button(fig_left, "Save left chart (PNG)", "risk_curves.png")   # safe if kaleido missing
    with cR:
        st.plotly_chart(fig_right, use_container_width=True, config=PLOT_CONFIG)
    ...
    st.plotly_chart(fig_env, use_container_width=True, config=PLOT_CONFIG)
    ...
    st.plotly_chart(fig_hl,  use_container_width=True, config=PLOT_CONFIG)
