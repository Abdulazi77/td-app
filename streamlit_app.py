...
Single-tab, linked workflow. Adds robust PNG download buttons (Kaleido) with HTML fallback.
"""
...
# ------------------------------ PNG download helper ------------------------------
def png_download_button(fig: go.Figure, label: str, fname: str = "plot.png",
                        scale: float = 2.0, width: int | None = None, height: int | None = None):
    """
    Render a Streamlit download button that returns a PNG of a Plotly figure.
    If Kaleido is unavailable (or errors), show a friendly note and offer an HTML fallback.
    """
    try:
        # these are safe even if kaleido is absent; they only apply if available
        ...
        buf = io.BytesIO(fig.to_image(format="png", scale=scale))
        st.download_button(
            label=label, data=buf.getvalue(),
            file_name=fname, mime="image/png"
        )
    except Exception as e:
        with st.expander(f"{label} (PNG export not available)"):
            st.caption("PNG export needs the 'kaleido' package. Providing an interactive HTML fallback.")
            html = io.BytesIO(fig.to_html(full_html=False).encode("utf-8"))
            st.download_button(
                label="Download interactive HTML",
                data=html, file_name=fname.replace(".png", ".html"), mime="text/html"
            )
...
def I_moment(od_in: float, id_in: float) -> float: return (math.pi / 64.0) * (od_in**4 - id_in**4)
...
with tab:
    ...
    png_download_button(fig3d, "Save 3D schematic (PNG)", "trajectory_3d.png", scale=2.0, width=1000, height=700)

    # 2D TVD–VS split by shoe
    ...
    png_download_button(fig2d, "Save 2D profile (PNG)", "trajectory_2d.png", scale=2.0, width=1200, height=500)
    ...
    if simple_mode:
        ...
        with c1:
            ...
            png_download_button(figT, "Save torque vs MD (PNG)", "torque_vs_md.png")
        with c2:
            st.plotly_chart(figH, use_container_width=True)
            png_download_button(figH, "Save hookload vs MD (PNG)", "hookload_vs_md.png")

    # ------------------ Safety overlays (optional) ------------------
    if not simple_mode:
        ...
        # Right plot — elemental torque with limits & combined-load
        fig_right = go.Figure()
        for mu in mu_band:
            dmu, tmu = run_td_off_bottom(mu)
            if tmu[0] < tmu[-1]:  # ensure surface at right
                tmu = tmu[::-1]
            fig_right.add_trace(go.Scatter(x=tmu/1000.0, y=dmu, name=f"μ={mu:.2f}", mode="lines"))

        # labels for sections (visual only)
        dp_top, dp_bot = 0.0, dp_len
        hwdp_top, hwdp_bot = dp_bot, dp_bot + hwdp_len
        dc_top, dc_bot = hwdp_bot, hwdp_bot + dc_len
        fig_right.add_hrect(y0=dp_top, y1=dp_bot, line_width=0, annotation_text="DP",
                            annotation_position="top left", opacity=0.0)
        fig_right.add_hrect(y0=hwdp_top, y1=hwdp_bot, line_width=0, annotation_text="HWDP",
                            annotation_position="top left", opacity=0.0)
        fig_right.add_hrect(y0=dc_top, y1=dc_bot, line_width=0, annotation_text="DC",
                            annotation_position="top left", opacity=0.0)
        ...
        # combined-load curve for pickup
        ...
        with cL:
            ...
            png_download_button(fig_left, "Save left chart (PNG)", "risk_curves.png", scale=2.0, width=1200, height=800)
        with cR:
            st.plotly_chart(fig_right, use_container_width=True)
            png_download_button(fig_right, "Save right chart (PNG)", "elemental_limits.png", scale=2.0, width=1200, height=800)
        ...
        fig_env.add_trace(go.Scatter(x=[abs(df_itr['M_next_lbf_ft'].to_numpy()[-1])/1000.0], y=[hookload[-1]/1000.0],
                                     mode="markers", name="Operating point", marker=dict(size=10)))
        ...
        with c3:
            ...
            png_download_button(fig_env, "Save TJ envelope (PNG)", "envelope.png", scale=2.0, width=1000, height=600)
        with c4:
            st.plotly_chart(fig_hl,  use_container_width=True)
            png_download_button(fig_hl, "Save Hookload diagnostics (PNG)", "hookload_diag.png", scale=2.0, width=1000, height=600)
