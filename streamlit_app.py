...
""")
    ...
    # subset of wellpath coordinates for the run interval (string in hole)
    E_run = E[start_idx:]
    N_run = N[start_idx:]
    TVD_run = TVD[start_idx:]

    # Drill string profile: shows DC / HWDP / DP along the actual wellpath
    st.subheader("Drill String Profile")
    depth_mid = 0.5*(md_run[:-1] + md_run[1:])
    tvd_mid   = 0.5*(TVD_run[:-1] + TVD_run[1:])

    fig_ds = go.Figure()
    comp_colors = {"DP": "#B0B0B0", "HWDP": "#707070", "DC": "#404040"}
    for comp_name in ["DP", "HWDP", "DC"]:
        mask = (comp_along == comp_name)
        if np.any(mask):
            fig_ds.add_trace(go.Scatter(
                x=tvd_mid[mask],
                y=depth_mid[mask],
                mode="lines",
                line=dict(width=14, color=comp_colors.get(comp_name, "#AAAAAA")),
                name=comp_name
            ))

    fig_ds.update_layout(
        title="Drill String Profile",
        xaxis_title="TVD (ft)",
    ...
    st.caption("**How to read:** Vertical stick representation of the drill string in the actual wellbore. Colours show DP, HWDP and DC sections along MD/TVD.")
    ...
    # Tension profile and surface torque overview at TMD
    st.subheader("Tension & Torque Overview at TMD")

    def run_tension_profile(scenario_name: str):
        df_s, T_s, _ = soft_string_stepper(
            md_run, inc_deg_run, kappa, cased_mask, comp_along, comp_props,
            mu_cased_slide, mu_open_slide, mu_cased_rot, mu_open_rot,
        ...
        T_mag = np.abs(df_s["T_next_lbf"].to_numpy())
        return depth_s, T_mag

    # Left: tension along the string for key operations
    fig_tension = go.Figure()
    tension_cases = [
        ("slackoff", "RiH", "#1f77b4"),
        ("pickup", "PoOH", "#d62728"),
        ("rotate_off", "RoB", "#2ca02c"),
        ("onbottom", "ROB", "#ffbf00"),
    ]
    for scen_name, label, color in tension_cases:
        depth_s, T_mag = run_tension_profile(scen_name)
    ...
    # Right: surface torque vs depth while rotating off-bottom
    df_rob, T_rob, M_rob = soft_string_stepper(
        md_run, inc_deg_run, kappa, cased_mask, comp_along, comp_props,
        mu_cased_slide, mu_open_slide, mu_cased_rot, mu_open_rot,
        mw_ppg, scenario="rotate_off", WOB_lbf=wob, Mbit_ftlbf=Mbit,
        tortuosity_mode=tort_mode, tau=tau, mu_open_boost=mu_boost
    )
    depth_rob = df_rob["md_bot_ft"].to_numpy()
    torque_rob = np.abs(df_rob["M_next_lbf_ft"].to_numpy())

    fig_torque_profile = go.Figure()
    fig_torque_profile.add_trace(go.Scatter(
        x=torque_rob,
        y=depth_rob,
        mode="lines",
        name="RiH",
    ...
    fig_torque_profile.update_layout(
        title="Surface Torques Rotating off Bottom  while drilling open hole section",
        xaxis_title="Torque (ft-lb)",
        yaxis_title="MD (ft)",
        yaxis=dict(autorange="reversed"),
        height=500,
        margin=dict(l=10, r=10, t=40, b=10),
    ...
    with c_prof2:
        st.plotly_chart(fig_torque_profile, use_container_width=True)
    st.caption("**How to read:** Left: axial tension vs depth for RiH, PoOH, rotating off-bottom (RoB) and rotating on-bottom (ROB). Right: torque distribution while rotating off-bottom in the current section.")
