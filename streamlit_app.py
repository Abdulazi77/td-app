...
def synth_horizontal(kop_md, br, lat_len, target_md, az_deg):
    ...
    for m in md:
        ...
        prev = inc[-1] if inc else 0.0
        if not built:
            nxt = min(prev + br*(ds/100.0), 90.0); built = abs(nxt-90.0) < 1e-12
            inc.append(nxt)
        else:
            inc.append(90.0)
    return md, inc, [az_deg]*len(md)
...
def solve_torque_drag(md, inc_deg, dls_deg_100,
    ...
    for i in range(n-1, 0, -1):
        ...
        # N = w_b sin(theta) + T * kappa
        N = w_b*math.sin(theta) + T[-1]*kap

        # Axial / torque increments
    ...
    return {
        "T_lbf_along": T, "M_lbf_ft_along": M,
        "trace_rows": rows,
...
with tab1:
    ...
    with b:
        ...
        az_deg = st.number_input("Azimuth (deg from North, clockwise)", 0.0, 360.0, az_map[az_quick], 1.0)
    ...
    if use_simple:
        ...
        if add[1].button("➕ Add interval"):
            st.session_state["prog_rows"] = min(12, st.session_state["prog_rows"]+1); st.rerun()
    ...
    if st.button("Compute trajectory & 3D schematic"):
        ...
        # Cased segments (blue)
        for (top, bot) in cased_intervals:
            idx = seg_idx(top, bot)
            if len(idx) < 2: continue
            if simple_sizes:
                width_px = max(2, int(2 + simple_sizes["casing_id_in"]/4.0))
        ...
        for (top, bot) in openhole_intervals:
            ...
            if simple_sizes:
                width_px = max(2, int(2 + simple_sizes['openhole_od_in']/4.0))
                label += f" (OD {simple_sizes['openhole_od_in']:.2f} in)"
            fig3d.add_trace(go.Scatter3d(
                x=[east[k] for k in idx], y=[north[k] for k in idx], z=[-tvd[k] for k in idx],
        ...
        B.plotly_chart(prof, use_container_width=True)
        B.plotly_chart(plan, use_container_width=True)
