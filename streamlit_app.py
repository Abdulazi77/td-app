...
# -*- coding: utf-8 -*-
"""
PEGN 517 — Torque & Drag (Δs = 1 ft)
Rebuilt, single-tab tool matching project guideline:
- Minimum-curvature survey (import or synthetic profiles)
- Soft-string T&D (Johancsik): PU, SO, Rotating(=avg(PU,SO)); Δs = 1 ft
- Off-bottom torque with rotating friction only
- μ sweep for sensitivity; TVD–VS and 3D wellpath
- Buckling indicators + Connection envelope, 0.8×MU gate
- Robust Neutral Point (on-bottom) with clear status
...
def Z_in3(od_in: float, id_in: float) -> float:
    return I_in4(od_in, id_in) / (od_in/2.0 if od_in>0 else 1.0)
...
def synth_build_hold_drop(kop_md, build_rate, theta_hold_deg, drop_rate, target_md, az_deg):
    ...
    start_drop = 0.75 * target_md  # simple template
...
def soft_string_stepper(
    ...
    mw_ppg: float,
    scenario: str = "slackoff",     # "pickup" | "slackoff" | "onbottom"
    ...
    include_slide_friction: bool = True,  # False for rotating torque-only run
):
    """
    Δs = 1 ft soft-string integration, bit -> surface.
      dT = (± w_b cosθ + μ_s N)*Δs      (if include_slide_friction else ± w_b cosθ*Δs)
      dM = (μ_r N r_eff)*Δs
      N  = w_b sinθ + T κ
    Contact radius r_eff ≈ pipe radius (soft-string).
    """
    ds = 1.0
    ...
    for i in range(nseg):
        ...
        # tortuosity penalties in open hole
        if not cased_seg[i]:
    ...
    # boundary condition at bit
    if scenario == "onbottom":
        T[0] = -float(WOB_lbf)       # compressive at bit
        M[0] = float(Mbit_ftlbf)     # motor/bit torque
    else:
        T[0] = 0.0; M[0] = 0.0

    # axial sign: + pulling up (pickup), - running in (slackoff/onbottom)
    sgn_weight = +1.0 if scenario == "pickup" else -1.0
...
# ───────────────────── Helper diagnostics ────────────────────────
def neutral_point_from_onbottom(md: np.ndarray, T_on: np.ndarray) -> Tuple[float, str]:
    """
    Return (NP_MD, status_message). T_on is bit->surface axial force for 'onbottom'.
    Positive = tension, Negative = compression. Linear interpolation at the sign change.
    """
    md = np.asarray(md); T = np.asarray(T_on)
    if len(md) < 2 or len(T) < 2:
        return float('nan'), "No data"
    if np.all(T <= 0):   # all compression
        return md[-1], "All compression (NP above surface)"
    if np.all(T >= 0):   # all tension
        return md[0], "All tension (NP at/below bit)"
    for i in range(len(T)-1):
        if T[i] == 0.0:
            return md[i], "Exact zero"
        if T[i] * T[i+1] < 0.0:
            frac = abs(T[i]) / (abs(T[i]) + abs(T[i+1]) + 1e-12)
            return md[i] + frac*(md[i+1] - md[i]), "Interpolated"
    return float('nan'), "No crossing found"

def add_bha_bands(fig: go.Figure, depth_edges: np.ndarray, comp: np.ndarray):
    """Shade DC/HWDP/DP bands across full x-range to match industry charts."""
    if len(depth_edges) == 0: return
    runs = []
    start = 0
    for i in range(1, len(comp)):
        if comp[i] != comp[i-1]:
            runs.append((comp[i-1], depth_edges[start], depth_edges[i-1]))
            start = i
    runs.append((comp[-1], depth_edges[start], depth_edges[-1]))
    colors = {"DC":"rgba(180,0,0,0.06)","HWDP":"rgba(0,140,0,0.06)","DP":"rgba(0,0,180,0.06)"}
    for c, y0, y1 in runs:
        fig.add_shape("rect", xref="paper", x0=0, x1=1, y0=y0, y1=y1, line=dict(width=0),
                      fillcolor=colors.get(c,"rgba(0,0,0,0.03)"))
        fig.add_annotation(xref="paper", x=0.99, y=0.5*(y0+y1), text=c, showarrow=False,
                           xanchor="right", yanchor="middle", font=dict(size=10))

# ─────────────────────────────── UI ──────────────────────────────
(tab,) = st.tabs(["Wellpath + Torque & Drag (linked)"])

with tab:
    st.header("Wellpath + Torque & Drag (Δs = 1 ft)")
    st.caption("Minimum-curvature survey; Johancsik soft-string; Δs=1 ft. "
               "Rotating off-bottom HL = average(PU, SO); Off-bottom torque uses rotating μ only.")

    # ───────── Survey: import or synthesize
    st.subheader("Trajectory — import CSV or synthesize (Minimum Curvature)")
    up = st.file_uploader("Upload survey CSV (columns: MD, INC, AZI). Leave empty to synthesize.", type=["csv"])

    if up is not None:
        try:
            df_svy = pd.read_csv(up)
            cols = {c.upper(): c for c in df_svy.columns}
            md = df_svy[cols[[k for k in cols if k=="MD"][0]]].to_numpy(dtype=float)
            inc_deg = df_svy[cols[[k for k in cols if k=="INC"][0]]].to_numpy(dtype=float)
            az_deg  = df_svy[cols[[k for k in cols if k=="AZI"][0]]].to_numpy(dtype=float)
            # resample to Δs=1 ft
        ...
        az0     = c4.number_input("Azimuth (deg, CW from North)", 0.0, 360.0, 0.0, 1.0)
    ...
    st.subheader("Casing / Open-hole (last string + open hole)")
    md_end = float(md[-1])
    ...
    vs_ref = st.number_input("VS reference azimuth (deg)", 0.0, 360.0, float(az[0]), 1.0)
    ...
    with st.expander("Typical μ starting ranges"):
        st.markdown("""
- **Casing (WBM):** 0.15–0.25  
- **Open-hole (WBM):** 0.25–0.40 (can be higher if hole cleaning is poor)  
- **OBM/SBM:** often lower than WBM  
*(Calibrate with history match for your section.)*
""")
    ...
    cA, cB = st.columns(2)
    cleaning_mode = cA.selectbox("Hole cleaning (μ booster in open hole)", ["Good (0)", "Fair (+0.03)", "Poor (+0.08)"], index=0)
    mu_boost = {"Good (0)":0.0, "Fair (+0.03)":0.03, "Poor (+0.08)":0.08}[cleaning_mode]
    tort_mode = cB.selectbox("Tortuosity penalty mode", ["off", "kappa", "mu"], index=0)
    ...
    # main runs (PU/SO for HL; rotating torque-only run)
    ...
    df_rot_tq, _, M_rot = soft_string_stepper(
        ...
        mu_cased_slide, mu_open_slide, mu_cased_rot, mu_open_rot, mw_ppg,
        scenario="slackoff", tortuosity_mode=tort_mode, tau=tau, mu_open_boost=mu_boost,
        include_slide_friction=False  # torque only (rotating μ)
    )

    # on-bottom (for NP)
    wob = st.number_input("WOB (lbf, on-bottom)", 0.0, 150000.0, 6000.0, 100.0)
    motor_on = st.checkbox("Motor on-bottom (bit torque from ΔP)", value=False)
    K_tbit = st.number_input("Motor torque factor K (lbf-ft/psi)", 0.0, 10_000.0, 2.5, 0.1) if motor_on else 0.0
    deltaP = st.number_input("Motor ΔP (psi)", 0.0, 5000.0, 0.0, 10.0) if motor_on else 0.0
    Mbit = K_tbit * deltaP if motor_on else 0.0
    df_on, T_on, _ = soft_string_stepper(
        md, inc_deg, kappa, (md<=shoe_md), comp_along, comp_props,
        mu_cased_slide, mu_open_slide, mu_cased_rot, mu_open_rot, mw_ppg,
        scenario="onbottom", WOB_lbf=wob, Mbit_ftlbf=Mbit,
    ...
    surf_torque = abs(M_rot[-1])  # rotating off-bottom torque
    st.success(f"Surface HL — PU: {HL_pick:,.0f} lbf | SO: {HL_so:,.0f} lbf | Rotating (avg): {HL_rot:,.0f} lbf")
    st.info(f"Surface torque (off-bottom, rotating μ): {surf_torque:,.0f} lbf-ft")

    # ───────── Safety & limits
    st.subheader("Safety & limits")
    s1, s2, s3 = st.columns(3)
    tj_name = s1.selectbox("Tool-joint size", list(TOOL_JOINT_DB.keys()), index=2)  # NC38 default
    sf_joint = s2.number_input("Safety factor (tool-joint)", 1.00, 2.00, 1.10, 0.05)
    sf_tension = s3.number_input("Safety factor (pipe tension)", 1.00, 2.00, 1.15, 0.05)
    ...
    # Neutral point
    NP_md, np_status = neutral_point_from_onbottom(md, np.array(T_on))
    NP_from_bit = md[-1] - NP_md
    st.write(f"Neutral point: **{NP_md:,.0f} ft MD** (status: {np_status}) → "
             f"{'✅ inside DCs' if NP_from_bit <= dc_len else '❌ not inside DCs'}")
    ...
    c1m.metric("BSR DP→HWDP", f"{BSR_dp_h:.2f}")
    c2m.metric("BSR HWDP→DC", f"{BSR_h_dc:.2f}")
    c3m.metric("SR DP→HWDP", f"{SR_dp_h:.2f}")
    c4m.metric("SR HWDP→DC", f"{SR_h_dc:.2f}")

    # ───────── Charts: Left = μ-sweep torque; Right = elemental torque & limits
    st.markdown("### T&D Model Charts — Risk curves and limits")
    mu_band = st.multiselect("μ sweep (same μ for slide/rot in sensitivity):", [0.15,0.20,0.25,0.30,0.35,0.40],
    ...
    def run_td_off_bottom(mu_slide: float, mu_rot: float):
        # Torque-only run (rotating). Axial sliding friction off.
    ...
    for mu in mu_band:
        ...
        # ensure correct plotting direction (depth top→bottom via y-axis reversed)
        if tmu[0] > tmu[-1]:  # cumulative should increase; if not, reverse both
            tmu = tmu[::-1]; dmu = dmu[::-1]
    ...
    # RIGHT: elemental torque (cumulative) + limits + combined-load curve
    fig_right = go.Figure()
    for mu in mu_band:
        dmu, tmu = run_td_off_bottom(mu, mu)
        # plot as elemental torque vs depth (same as left, different annotations)
        if tmu[0] > tmu[-1]:
            tmu = tmu[::-1]; dmu = dmu[::-1]
    ...
    # combined-load curve using pickup axial force (tension only)
    ...
    fig_right.add_trace(go.Scatter(x=T_allow/1000.0, y=df_pick["md_bot_ft"].to_numpy(),
                                   mode="lines", name="TJ combined-load (approx)", line=dict(dash="dot")))
    ...
    # Envelope & Hookload diagnostics (buckling + rig limit)
    Epsi = 30.0e6
    Iin4_dp = I_in4(dp_od, dp_id)
    rbore_ft = 0.5*hole_diam_in*IN2FT; rpipe_ft = 0.5*dp_od*IN2FT
    clearance_ft = max(1e-3, rbore_ft - rpipe_ft)
    theta_rad = np.deg2rad(np.maximum(0.0, df_pick['inc_deg'].to_numpy()))
    Fs = 2.0*np.sqrt(np.maximum(0.0, Epsi*Iin4_dp) * df_pick['w_b_lbft'].to_numpy()*np.sin(theta_rad)/clearance_ft)
    Fh = 1.6*Fs  # indicator
    ...
    fig_env.add_trace(go.Scatter(x=T_env/1000.0, y=F_env/1000.0, mode="lines", name="Connection envelope (approx)"))
    ...
    fig_hl = go.Figure()
    fig_hl.add_trace(go.Scatter(x=np.abs(df_pick['T_next_lbf'].to_numpy())/1000.0, y=depth, mode="lines", name="HL Pickup"))
    fig_hl.add_trace(go.Scatter(x=np.abs(df_so['T_next_lbf'].to_numpy())/1000.0,   y=depth, mode="lines", name="HL Slack-off"))
    fig_hl.add_trace(go.Scatter(x=(0.5*(np.abs(df_pick['T_next_lbf'].to_numpy())+np.abs(df_so['T_next_lbf'].to_numpy())))/1000.0,
                                y=depth, mode="lines", name="HL Rotating (avg)", line=dict(dash="dot")))
    ...
    def grid_calibrate_mu(md, inc_deg, kappa, cased_mask, comp_along, comp_props, mw_ppg,
                          depth_for_fit, measured_pickup_hl, measured_slackoff_hl,
                          measured_rotate_hl, measured_surface_torque,
            ...
            for mu_o_s in mu_o_s_rng:
                for mu_c_r in mu_c_r_rng:
                    for mu_o_r in mu_o_r_rng:
                        err2 = 0.0
                        # PU / SO
                        _, T_pick_fit, _ = soft_string_stepper(
                        ...
                        _, T_so_fit, _ = soft_string_stepper(
                            md_fit, inc_fit, kappa_fit, cased_fit, comp_fit, comp_props,
                            mu_c_s, mu_o_s, mu_c_r, mu_o_r, mw_ppg,
                            scenario="slackoff", include_slide_friction=True
                        )
                        HL_pick_f = abs(T_pick_fit[-1]); HL_so_f = abs(T_so_fit[-1]); HL_rot_f = 0.5*(HL_pick_f + HL_so_f)
                        if measured_pickup_hl:
                            err2 += (HL_pick_f - measured_pickup_hl)**2
                        if measured_slackoff_hl:
                            err2 += (HL_so_f   - measured_slackoff_hl)**2
                        if measured_rotate_hl:
                            err2 += (HL_rot_f  - measured_rotate_hl)**2
                        if measured_surface_torque:
                            # torque-only
                            _, _, M_rot_fit = soft_string_stepper(
                                md_fit, inc_fit, kappa_fit, cased_fit, comp_fit, comp_props,
                                mu_c_s, mu_o_s, mu_c_r, mu_o_r, mw_ppg,
                                scenario="slackoff", include_slide_friction=False
                            )
                            err2 += (abs(M_rot_fit[-1]) - measured_surface_torque)**2
                        if err2 < best_err:
                            best_err = err2
    ...
    if st.button("Run μ calibration (grid search)"):
        try:
            ...
            mu_ranges = {"mu_c_s": parse(mu_c_s_rng), "mu_o_s": parse(mu_o_s_rng),
    ...
    st.download_button("Download elemental results (xlsx)", data=out.getvalue(),
                       file_name="TD_iteration_results.xlsx",
                       mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    st.caption("Implements the project outputs: TVD–VS, PU/SO/ROT drags vs depth, rotating torque vs depth, "
               "μ-sweep, 3D wellpath, buckling indicators, and a torque safety envelope. Δs = 1 ft.")
