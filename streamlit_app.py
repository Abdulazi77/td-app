# I'll write an updated `streamlit_app.py` into a plain text file that adds the slide-style Neutral Point formula
...
\"\"\")
    ...
    if math.isnan(np_md):
        st.warning(\"Neutral point not found by sign-change. Using slide formula (BF × L).\", icon=\"⚠️\")
    ...
    if simple_mode:
        ...
        fig_drag.add_trace(go.Scatter(x=Fs/1000.0, y=depth, mode=\"lines\", name=\"Fs (sinusoidal)\", line=dict(dash=\"dash\")))
        fig_drag.add_trace(go.Scatter(x=Fh/1000.0, y=depth, mode=\"lines\", name=\"Fh (helical)\", line=dict(dash=\"dot\")))

        fig_drag.add_vline(x=F_tensile_sf/1000.0, line_color=\"crimson\", line_dash=\"dash\", annotation_text=\"Pipe tension limit / SF\", annotation_position=\"top\")
    ...
    st.caption(\"Johancsik soft-string (Δs=1 ft). Survey → shoe → T&D are linked. Defaults: last casing 9-5/8, OH 8.50 in. \"
               \"Tools include history-matching μ, calibrated overlay, NP check (model + slide formula), 0.8×MU gate, BSR/SR, tortuosity penalty, motor bit torque, \"
               \"and rig-limit margins, plus Menand-style buckling loads, drag μ-sensitivity, operation comparisons, and a BSI hot-spot view.\")'''
