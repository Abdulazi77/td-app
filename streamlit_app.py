...
# Classic “T&D Model Chart” view by default (μ-sweep + safety lines).
# Methods: minimum curvature + Johancsik soft-string; BF = (65.5 − MW)/65.5.
...
# ------------------------------ minimal standards (ID from standards only) ----
# (Trim/extend as needed; IDs are from standard tables.)
CASING_DB = {
    "13-3/8": {"weights": {48.0: 12.415, 54.5: 12.347, 61.0: 12.107}, "grades": ["J55","K55","L80","P110"]},
    "9-5/8":  {"weights": {36.0: 8.535, 40.0: 8.321, 43.5: 8.097},  "grades": ["J55","L80","P110"]},
...
# Tool-joint mini-DB (for reference lines)
TOOL_JOINT_DB = {
...
def soft_string_stepper(md, inc_deg, kappa_rad_per_ft, cased_mask,
    ...
    if kappa_arr.shape[0] == md.shape[0]: kappa_seg = kappa_arr[:-1]
    else: kappa_seg = kappa_arr
    nseg = md.shape[0] - 1
    ...
    for i in range(nseg):
        comp = comp_along_depth[i]                   # 'DC'/'HWDP'/'DP'
    ...
    for i in range(nseg):
        ...
        dT[i] = T_next - T[i]; T[i+1] = T_next
        M_next = M[i] + (mu_r[i]*N_side[i]*r_eff_ft[i])*ds
        dM[i] = M_next - M[i]; M[i+1] = M_next
...
with tab1:
    ...
    quick = c3.selectbox("Quick azimuth", ["North (0)","East (90)","South (180)","West (270)"], index=0)
    quick_map = {"North (0)":0.0,"East (90)":90.0,"South (180)":180.0,"West (270)":270.0}
    az_quick_deg = quick_map[quick]
    az_deg_num = c4.number_input("Azimuth (deg, from North, clockwise)", 0.0, 359.99, az_quick_deg, 0.5)
    az_deg = float(az_deg_num)

    c5, c6, c7 = st.columns(3)
    br = c5.number_input("Build rate (deg/100 ft)", 0.0, 20.0, 3.0, 0.1)
    if profile == "Build & Hold":
        theta_hold = c6.number_input("Final inclination (deg)", 0.0, 90.0, 38.5, 0.5)
        ...
        theta_hold = c6.number_input("Hold inclination (deg)", 0.0, 90.0, 30.0, 0.5)
        drop_rate  = c7.number_input("Drop rate (deg/100 ft)", 0.0, 20.0, 2.0, 0.1)
        ...
        lateral    = c6.number_input("Lateral length (ft)", 0.0, 30000.0, 2000.0, 100.0)
        target_md  = c7.number_input("Target MD (ft)", 100.0, 100000.0, 10000.0, 100.0)
        md, inc_deg, az = synth_horizontal(kop_md, br, lateral, target_md, az_deg)

    N, E, TVD, DLS = mincurv_positions(md, inc_deg, az)

    # Preview shoe for schematic (use last saved if present)
    shoe_preview = st.number_input("Preview shoe MD for schematic (ft)", 0.0, float(md[-1]), 
                                   float(st.session_state.get("shoe_md", 3000.0)), 50.0)
    cased_mask_preview = md <= shoe_preview

    # 3D plot: cased (thicker blue) vs open-hole (brown)
    fig3d = go.Figure()
    fig3d.add_trace(go.Scatter3d(
        x=E[cased_mask_preview], y=N[cased_mask_preview], z=-TVD[cased_mask_preview],
        mode="lines", line=dict(width=6, color="#4ea1ff"), name="Cased"
    ))
    fig3d.add_trace(go.Scatter3d(
        x=E[~cased_mask_preview], y=N[~cased_mask_preview], z=-TVD[~cased_mask_preview],
    ...
    st.caption("Minimum curvature positions with ratio factor and DLS. Depth-down convention in 3D.")

    # share to the T&D tab
    st.session_state["md"] = md
    st.session_state["inc_deg"] = inc_deg
    st.session_state["az_deg"] = az
    st.session_state["dls"] = DLS

# ======== Torque & Drag ========
with tab2:
    if any(k not in st.session_state for k in ["md","inc_deg","az_deg","dls"]):
        st.warning("Build a trajectory first in the left tab.")
        st.stop()

    md       = st.session_state["md"]
    inc_deg  = st.session_state["inc_deg"]
    az       = st.session_state["az_deg"]
    DLS      = st.session_state["dls"]
    kappa    = (DLS*DEG2RAD)/100.0   # rad/ft per station

    st.subheader("Casing / Open-hole (simple mode)")
    cc1, cc2, cc3, cc4, cc5 = st.columns([1,1,1,1,1])
    # Default casing = 9-5/8"
    nominal = cc1.selectbox("Last casing nominal OD", list(CASING_DB.keys()), index=1)
    weight_list = list(CASING_DB[nominal]["weights"].keys())
    # Default to a common weight (e.g., 36# for 9-5/8)
    default_weight_index = weight_list.index(36.0) if (nominal=="9-5/8" and 36.0 in weight_list) else 0
    weight  = cc2.selectbox("lb/ft (standards only)", weight_list, index=default_weight_index)
    casing_id_in = CASING_DB[nominal]["weights"][weight]
    cc3.text_input("Casing ID (in, from table)", f"{casing_id_in:.3f}", disabled=True)
    shoe_md = cc4.number_input("Deepest shoe MD (ft)", 0.0, float(md[-1]), float(st.session_state.get("shoe_md", 3000.0)), 50.0)
    ...
    # map bit→surface component per segment
    nseg = len(md) - 1
    ...
    # toggle: classic vs model-chart; default = model-chart (Dr’s style)
    simple_mode = st.checkbox("Use classic simple view (hide risk overlays)", value=False)

    scen = st.selectbox("Scenario", ["Slack-off (RIH)","Pickup (POOH)","Rotate off-bottom","Rotate on-bottom"])
    scenario = "slackoff" if "Slack" in scen else "pickup" if "Pickup" in scen else ("rotate_off" if "off" in scen else "onbottom")
    ...
    surf_hookload = max(0.0, -T_arr[-1])
    surf_torque   = abs(M_arr[-1])
    st.success(f"Surface hookload: {surf_hookload:,.0f} lbf — Surface torque: {surf_torque:,.0f} lbf-ft")

    if simple_mode:
        figT = go.Figure(go.Scatter(x=depth, y=np.abs(df_itr["M_next_lbf_ft"]), mode="lines", name="Torque"))
        figT.update_xaxes(title_text="MD (ft)"); figT.update_yaxes(title_text="Torque (lbf-ft)")
        figH = go.Figure(go.Scatter(x=depth, y=np.maximum(0.0, -df_itr["T_next_lbf"]), mode="lines", name="Hookload"))
        ...
        sf_joint   = s2.number_input("Safety factor", 1.00, 2.00, 1.10, 0.05)
        mu_band    = s3.multiselect("μ sweep (off-bottom)", [0.15,0.20,0.25,0.30,0.35,0.40], default=[0.20,0.25,0.30,0.35])

        # Two cyan reference lines like the slide
        c1, c2 = st.columns(2)
        tube_kft     = c1.number_input("Tube make-up torque (k lbf-ft)", 1.0, 100.0, 16.0, 0.5)
        combined_kft = c2.number_input("Combined load torque (k lbf-ft)", 1.0, 100.0, 20.0, 0.5)

        tj = TOOL_JOINT_DB[tj_name]
        T_tube_sf     = (tube_kft*1000.0)/sf_joint
        T_combined_sf = (combined_kft*1000.0)/sf_joint

        # μ-sweep helper (rotate off-bottom)
        ...
        # Left: off-bottom risk curves
        fig_left = go.Figure()
        for mu in mu_band:
            dmu, tor = run_td_off_bottom(mu)
            fig_left.add_trace(go.Scatter(x=tor/1000.0, y=dmu, name=f"μ={mu:.2f}", mode="lines"))
        fig_left.add_vline(x=T_tube_sf/1000.0,     line_color="#00d5ff", line_dash="dash",
                           annotation_text="Tube torque / SF", annotation_position="top left")
        fig_left.add_vline(x=T_combined_sf/1000.0, line_color="#00d5ff",
                           annotation_text="Combined / SF",   annotation_position="top left")
        fig_left.update_yaxes(autorange="reversed", title_text="Depth / MD (ft)")
        fig_left.update_xaxes(title_text="Off-bottom torque (k lbf-ft)")
        fig_left.update_layout(height=680, margin=dict(l=8,r=8,t=30,b=8), legend=dict(orientation="h"))

        # Right: elemental torque stacked + same reference lines
        ...
        fig_right.add_vline(x=T_tube_sf/1000.0,     line_color="#00d5ff", line_dash="dash",
                            annotation_text="Tube / SF", annotation_position="top left")
        fig_right.add_vline(x=T_combined_sf/1000.0, line_color="#00d5ff",
                            annotation_text="Combined / SF", annotation_position="top left")
        fig_right.update_yaxes(autorange="reversed", title_text="Depth / MD (ft)")
        fig_right.update_xaxes(title_text="Elemental torque (k lbf-ft)")
        fig_right.update_layout(height=680, margin=dict(l=8,r=8,t=30,b=8), legend=dict(orientation="h"))
    ...
    st.caption("Johancsik soft-string with N ≈ w_b·sinθ + T·κ (Δs=1 ft). BF=(65.5–MW)/65.5. "
               "Trajectory drives all T&D results; depth-down plots and 3D schematic.")
