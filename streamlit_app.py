...
Linked workflow with calibration, NP (robust), 0.8×MU, BSR/SR, tortuosity, motor-BT,
...
def bf_from_mw(mw_ppg: float) -> float:
    # buoyancy factor (steel ~65.5 ppg)
...
def Z_in3(od_in: float, id_in: float) -> float:
    return I_in4(od_in, id_in) / max(od_in/2.0, EPS)
...
def soft_string_stepper(
    ...
    # accept either per-station or per-segment input
    if len(kappa_all) == len(md):
        kappa_seg = kappa_all[:-1].copy()
    else:
        kappa_seg = np.resize(kappa_all, nseg).astype(float)
...
def neutral_point_md(md: np.ndarray, T_arr: np.ndarray) -> float:
    ...
    for i in range(len(T_arr)-1):
        ...
        if abs(t1) < 1e-6: return md[i]
        if t1*t2 < 0:
            frac = abs(t1)/(abs(t1)+abs(t2)+1e-12)
            return md[i] + frac*(md[i+1]-md[i])
    return float('nan')

def neutral_point_frictionless(md: np.ndarray, inc_deg: np.ndarray, w_b_lbft: np.ndarray, WOB_lbf: float) -> float:
    """
    Approximate NP ignoring friction & curvature: find MD where cumulative
    buoyant weight projection equals WOB. Bit→surface accumulation.
    """
    inc = np.deg2rad(np.maximum(0.0, inc_deg[:-1]))
    dmd = np.diff(md)
    axial_per_ft = w_b_lbft*np.cos(inc)  # lbf/ft toward surface
    cum = np.cumsum(axial_per_ft * dmd)  # lbf
    # NP when cum >= WOB
    idx = np.searchsorted(cum, max(WOB_lbf,0.0))
    if idx >= len(dmd): return float('nan')
    # linear interpolate inside the segment
    prev = cum[idx-1] if idx>0 else 0.0
    need = max(WOB_lbf,0.0) - prev
    add = axial_per_ft[idx]*dmd[idx] if idx < len(dmd) else 1.0
    frac = 0.0 if add<=0 else clamp(need/max(add,1e-9),0.0,1.0)
    return md[idx] + frac*(md[idx+1]-md[idx])
...
> Calibrate to your well/section using the tool below.
""")
    ...
    # map string along depth (bit -> surface)
    nseg = len(md) - 1
    ...
    # curvature per-ft
    _, _, _, DLS_calc = mincurv_positions(md, inc_deg, az)
    kappa = (DLS_calc*DEG2RAD)/100.0
    ...
    surf_hookload = abs(T_arr[-1])  # magnitude of surface axial force
    ...
    st.caption("Stay ≤ ~80% of tool-joint make-up torque for margin to yield/fatigue.")

    # Neutral point — robust
    ...
    if math.isnan(np_md):
        ...
        np_md_fb = neutral_point_frictionless(md, inc_deg, df_on['w_b_lbft'].to_numpy(), wob)
        if math.isnan(np_md_fb):
            st.warning("Neutral point not found (WOB exceeds total buoyant axial support). Reduce WOB and/or increase DC length to bring NP into DCs.")
        else:
            NP_from_bit = md[-1] - np_md_fb
            in_DCs = NP_from_bit <= dc_len
            st.write(f"Friction-free NP ≈ **{NP_from_bit:,.0f} ft** from bit → {'✅ in DCs' if in_DCs else '❌ not in DCs'}")
            if not in_DCs: st.warning("Increase DC length and/or reduce WOB/μ to place NP inside the collars.")
    else:
        NP_from_bit = md[-1] - np_md
        in_DCs = NP_from_bit <= dc_len
        st.write(f"Neutral point depth from bit ≈ **{NP_from_bit:,.0f} ft** → {'✅ inside DCs' if in_DCs else '❌ not inside DCs'}")
        if not in_DCs:
            st.warning("Increase DC length so NP sits inside the collars for current WOB / friction.")
    ...
    c1m.metric("BSR DP→HWDP", f"{BSR_dp_h:.2f}", help="Bending Strength Ratio")
    c2m.metric("BSR HWDP→DC", f"{BSR_h_dc:.2f}")
    c3m.metric("SR DP→HWDP", f"{SR_dp_h:.2f}", help="Stiffness Ratio (I ratio)")
    ...
    if simple_mode:
        ...
        st.markdown("### T&D Model Chart — Risk curves and limits (off-bottom & elemental)")
        ...
        def run_td_off_bottom(mu_slide: float, mu_rot: float):
            ...
            d = df_tmp["md_bot_ft"].to_numpy()
            t = np.abs(df_tmp["M_next_lbf_ft"].to_numpy())
            # enforce monotonic with depth for plotting
            order = np.argsort(d)
        ...
        # RIGHT: elemental torque + limits (show combined-load limit vs depth)
        fig_right = go.Figure()
        for mu in mu_band:
            dmu, tmu = run_td_off_bottom(mu, mu)
        ...
        # Combined-load tool-joint limit using pickup tension
        ...
        # simple ellipse: (T/Ty)^2 + (F/Ft)^2 = 1
        ...
        Fs = 2.0*np.sqrt(np.maximum(Epsi*Iin4_dp,0.0) * np.maximum(df_itr['w_b_lbft'].to_numpy(),0.0)*np.sin(theta)/clearance_ft)
        Fh = 1.6*Fs

        fig_env = go.Figure()
        F_env = np.linspace(0, max(TOOL_JOINT_DB[tj_name]['F_tensile_lbf']/sf_tension,1.0), 100)
        ...
        fig_env.add_vline(x=0.8*T_makeup/1000.0, line_color="#00d5ff", line_dash="dash", annotation_text="0.8×MU", annotation_position="top")
        ...
        fig_hl.add_vline(x=rig_pull_lim/1000.0, line_color="magenta", line_dash="dot", annotation_text="Rig pull limit", annotation_position="top")
    ...
    st.caption("Johancsik soft-string (Δs=1 ft). Survey → shoe → T&D are linked. Tools include μ calibration, calibrated overlay, NP check with fallback, 0.8×MU gate, BSR/SR, tortuosity penalty, motor bit torque, and rig-limit margins.")
