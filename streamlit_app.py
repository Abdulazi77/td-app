...
# Classic view + Dr-style “T&D Model Chart” safety overlays (default ON).
#
# Methods:
# - Minimum curvature with ratio factor (survey positions, DLS).
# - Johancsik soft-string: N ≈ w_b·sinθ + T·κ, axial & torsional increments integrated bit→surface.
# - Buoyancy BF = (65.5 − MW)/65.5 (steel ≈ 65.5 ppg).
# - API RP 7G-like tension–torque envelope overlay (ellipse surrogate) + make-up torque / SF.
#
...
# Buoyancy factor for steel in mud (steel ≈ 65.5 ppg).
def bf_from_mw(mw_ppg):
    return (65.5 - mw_ppg)/65.5

# ------------------------------ standards (ID from tables) ---------------------
CASING_DB = {
    "13-3/8": {
...
# Tool-joint mini-db for overlays (approximate; extend as needed).
TOOL_JOINT_DB = {
...
def soft_string_stepper(md, inc_deg, kappa_rad_per_ft, cased_mask,
                        ...
                        hole_od_in=8.5):
    """
    Johancsik soft-string, Δs = 1 ft. All per-segment arrays must be length nseg=len(md)-1.
    """
    ds = 1.0
    md = np.asarray(md)
    ...
    if kappa_arr.shape[0] == md.shape[0]:
        kappa_seg = kappa_arr[:-1]
    else:
        kappa_seg = kappa_arr

    nseg = md.shape[0] - 1
    if nseg <= 0:
        raise ValueError("Need at least two MD stations for T&D.")

    inc = np.deg2rad(inc_deg[:-1])

    r_eff_ft = np.zeros(nseg)
    w_air = np.zeros(nseg)     # lb/ft (air)
    w_b   = np.zeros(nseg)     # buoyed
    mu_s  = np.zeros(nseg)
    mu_r  = np.zeros(nseg)

    BF = bf_from_mw(mw_ppg)
    BF_arr = np.full(nseg, BF, dtype=float)
    ...
    T = np.zeros(nseg+1)       # axial tension (lbf), index 0 at bit
    M = np.zeros(nseg+1)       # torque (lbf-ft)
    dT = np.zeros(nseg)
    dM = np.zeros(nseg)
    N_side = np.zeros(nseg)
    ...
    # axial sign (+ pickup, − slackoff, 0 rotate)
...
def api7g_envelope_points(F_lim, T_lim, n=120):
    F = np.linspace(0, max(F_lim, 1.0), n)
    T = T_lim*np.sqrt(np.clip(1.0 - (F/ max(F_lim, 1.0))**2, 0.0, 1.0))
    return F, T

# Buckling thresholds (rule-of-thumb helical ≈ 1.6× sinusoidal)
...
with tab1:
    ...
    # 3D trajectory — FIXED ORIENTATION (z = TVD, depth-down via reversed axis)
    # Also color cased vs open-hole if shoe depth known (from T&D tab).
    shoe_for_plot = st.session_state.get("shoe_md", None)
    fig3d = go.Figure()
    if shoe_for_plot is None:
        fig3d.add_trace(go.Scatter3d(
            x=E, y=N, z=TVD, mode="lines",
            line=dict(width=7, color="#4cc9f0"),
            name="Wellpath"
        ))
    else:
        mask_cased = md <= float(shoe_for_plot)
        fig3d.add_trace(go.Scatter3d(
            x=E[mask_cased], y=N[mask_cased], z=TVD[mask_cased], mode="lines",
            line=dict(width=7, color="#4cc9f0"), name="Cased"
        ))
        fig3d.add_trace(go.Scatter3d(
            x=E[~mask_cased], y=N[~mask_cased], z=TVD[~mask_cased], mode="lines",
            line=dict(width=7, color="#b88746"), name="Open hole"
        ))
    fig3d.update_layout(
        height=520,
        scene=dict(
            xaxis_title="East (ft)", yaxis_title="North (ft)", zaxis_title="TVD (ft)",
            zaxis=dict(autorange="reversed")  # depth-down view
        ),
    ...
    st.caption("Minimum curvature positions with ratio factor and DLS.")
...
with tab2:
    ...
    # Default last casing to 9-5/8 as requested
    nominal_list = list(CASING_DB.keys())
    default_index = max(0, nominal_list.index("9-5/8")) if "9-5/8" in nominal_list else 0
    nominal = cc1.selectbox("Last casing nominal OD", nominal_list, index=default_index)
    ...
    # Make shoe depth available to the 3D plot (Tab 1 coloring)
    ...
    # Compose per-depth component map (bit up) — exactly nseg entries
    nseg = len(md) - 1
    comp_along = np.empty(nseg, dtype=object)
    for i in range(nseg):
        # distance up from bit to segment top
    ...
    kappa = (DLS*DEG2RAD)/100.0  # rad/ft (station-based)

    # --------------------------------------------------------------------------
    # DEFAULT: Dr-style chart (safety overlays) -> simple_mode=False
    ...
    scen = st.selectbox("Scenario", ["Slack-off (RIH)","Pickup (POOH)","Rotate off-bottom","Rotate on-bottom"],
                        index=2)   # default to off-bottom like the Dr. slide
    ...
    if simple_mode:
        # Classic/simple plots (no limits)
        ...
        # ------------------ Safety overlays (Dr-style model chart) --------------
        ...
        F_env, T_env = api7g_envelope_points(F_tensile_sf, T_yield_sf, n=120)
        ...
        def run_td_off_bottom(mu):
            df_tmp, T_tmp, M_tmp = soft_string_stepper(
                md, inc_deg, kappa, (md<=shoe_md), comp_along, comp_props,
                mu, mu, mu, mu, mw_ppg,
                scenario="rotate_off", WOB_lbf=0.0, Mbit_ftlbf=0.0, hole_od_in=hole_diam_in
            )
            depth_mu = df_tmp["md_bot_ft"].to_numpy()
            tor_mu   = np.abs(df_tmp["M_next_lbf_ft"].to_numpy())
            return depth_mu, tor_mu

        # Left panel: off-bottom μ-sweep risk curves (torque vs depth)
        risk_traces = []
        for mu in mu_band:
            dmu, tmu = run_td_off_bottom(mu)
            risk_traces.append((mu, dmu, tmu))

        fig_left = go.Figure()
        for mu, dmu, tor in risk_traces:
        ...
        # Right panel: elemental torque + limits
        ...
        # Envelope & hookload diagnostics (buckling)
        Epsi = 30.0e6   # psi
        Iin4 = I_moment(dp_od, dp_id)  # DP dominant
