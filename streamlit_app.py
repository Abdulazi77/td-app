...
def bf_from_mw(mw_ppg: float) -> float:
    # steel ~65.5 ppg → buoyancy factor
...
# ------------------------------ survey builders -------------------
def synth_build_hold(
    kop_md: float,
    build_rate_deg_per_100ft: float,
    theta_hold_deg: float,
    target_md: float,
    az_deg: float,
...
def synth_build_hold_drop(
    ...
    start_drop = 0.75 * target_md  # simple synthetic pattern
    theta = np.maximum(0.0, theta - np.maximum(0.0, md - start_drop) * dr)
    az = np.full_like(md, az_deg, dtype=float)
    return md, theta, az

def synth_horizontal(
    kop_md: float,
    build_rate: float,
...
def mincurv_positions(md: Iterable[float], inc_deg: Iterable[float], az_deg: Iterable[float]):
    ...
    DLS = np.zeros(n)
    for i in range(1, n):
        I1 = inc_deg[i - 1] * DEG2RAD
        A1 = az_deg[i - 1] * DEG2RAD
        I2 = inc_deg[i] * DEG2RAD
        A2 = az_deg[i] * DEG2RAD
        dmd = ds[i - 1]
...
def soft_string_stepper(
    ...
    if len(kappa_seg) != nseg:
        kappa_seg = np.resize(kappa_seg, nseg)  # keeps app resilient to shape mishaps
    ...
    for i in range(nseg):
        comp = comp_arr[i]
        od_in = float(comp_props[comp]["od_in"])
        id_in = float(comp_props[comp]["id_in"])
        w_air_ft = float(comp_props[comp]["w_air_lbft"])
        w_air[i] = w_air_ft
        w_b[i] = w_air_ft * BF
        r_eff_ft[i] = 0.5 * od_in * IN2FT
        if cased_seg[i]:
            mu_s[i] = mu_slide_cased
            mu_r[i] = mu_rot_cased
        else:
            mu_s[i] = mu_slide_open
            mu_r[i] = mu_rot_open

    T = np.zeros(nseg + 1)  # axial tension (+) or compression (-); app uses negative for WOB
...
with tab:
    ...
    if idx > 1:
        fig3d.add_trace(
            go.Scatter3d(
                ...
                z=-TVD[:idx],
                mode="lines",
                line=dict(width=6, color="#4cc9f0"),
                name="Cased",
            )
        )
    if idx < len(md):
        fig3d.add_trace(
            go.Scatter3d(
                ...
                z=-TVD[idx - 1 :],
                mode="lines",
                line=dict(width=4, color="#a97142"),
                name="Open-hole",
            )
        )
    fig3d.update_layout(
        height=420,
        scene=dict(xaxis_title="East (ft)", yaxis_title="North (ft)", zaxis_title="TVD (ft)", zaxis=dict(autorange="reversed")),
    ...
    scenario = {
        "Slack-off (RIH)": "slackoff",
        "Pickup (POOH)": "pickup",
        "Rotate off-bottom": "rotate_off",
        "Rotate on-bottom": "onbottom",
    }[scen]
    ...
    if simple_mode:
        figT = go.Figure(
            go.Scatter(x=df_itr["md_bot_ft"], y=np.abs(df_itr["M_next_lbf_ft"]), mode="lines", name="Torque")
        )
        figT.update_xaxes(title_text="MD (ft)")
        figT.update_yaxes(title_text="Torque (lbf-ft)")
        figH = go.Figure(
            go.Scatter(x=df_itr["md_bot_ft"], y=np.maximum(0.0, -df_itr["T_next_lbf"]), mode="lines", name="Hookload")
        )
    ...
    if not simple_mode:
        ...
        mu_band = st.multiselect(
            "μ sweep for off-bottom risk curves", [0.15, 0.20, 0.25, 0.30, 0.35, 0.40], default=[0.20, 0.25, 0.30, 0.35]
        )

        tj = TOOL_JOINT_DB[tj_name]
        T_makeup_sf = tj["T_makeup_ftlbf"] / sf_joint
        T_yield_sf = tj["T_yield_ftlbf"] / sf_joint
        # IMPORTANT: pipe body tension should use its own SF (bug fix)
        ...
        fig_left.add_vline(
            x=T_makeup_sf / 1000.0, line_color="#00d5ff", line_dash="dash", annotation_text="Make-up torque / SF"
        )
        ...
        fig_right.add_trace(
            go.Scatter(x=tor_total / 1000.0, y=depth, name="Total", mode="lines", line=dict(width=3))
        )
        fig_right.add_vline(
            x=T_makeup_sf / 1000.0, line_color="#00d5ff", line_dash="dash", annotation_text="Make-up / SF"
        )
        fig_right.add_vline(
            x=rig_torque_lim / 1000.0, line_color="magenta", line_dash="dot", annotation_text="Top-drive limit"
        )
        ...
        rbore_ft = 0.5 * hole_diam_in * IN2FT
        ...
        fig_env = go.Figure()
        fig_env.add_trace(
            go.Scatter(x=(T_env) / 1000.0, y=(F_env) / 1000.0, mode="lines", name="API 7G envelope (approx)")
        )
        fig_env.add_vline(
            x=T_makeup_sf / 1000.0, line_color="#00d5ff", line_dash="dash", annotation_text="Make-up / SF"
        )
        ...
        fig_hl.add_vline(
            x=rig_pull_lim / 1000.0, line_color="magenta", line_dash="dot", annotation_text="Rig pull limit"
        )
...
# requirements.txt
# Pin stable versions for Streamlit Cloud or local installs.
# streamlit>=1.36
# plotly>=5.18
# numpy>=1.24
# pandas>=2.0
