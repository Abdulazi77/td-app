...
def soft_string_stepper(
    ...
    # axial direction factor
    if scenario == "onbottom":
        sgn_ax = -1.0               # like slackoff (compression from bit upward)
    else:
        sgn_ax = {"pickup": +1.0, "slackoff": -1.0}.get(scenario, 0.0)

    # sliding only in certain scenarios
...
""")
    ...
    if simple_mode:
        ...
        # FIX 1: use casing_id_in directly (no 2×)
        ...
        # FIX 2: Menand Fs/Fh with sqrt(EI·w')
        w_b_local = df_itr['w_b_lbft'].to_numpy()
        Fs = 2.0  * np.sqrt(EI_ft2_local * w_b_local) / denom
        Fh = 2.83 * np.sqrt(EI_ft2_local * w_b_local) / denom
        ...
        fig_buck.add_trace(go.Scatter(
            ...
            mode="lines", name="Compressive load (slackoff)",
        ...
        fig_buck.update_layout(
            title="Compression vs critical buckling loads — sliding / RIH",
            height=500,
            margin=dict(l=10, r=10, t=40, b=10),
            legend=dict(orientation="h")
        )
        ...
        fig_bsi.update_layout(
            title="Hookload & Buckling Severity Index while sliding",
            height=500,
            margin=dict(l=10, r=10, t=40, b=10),
            legend=dict(orientation="h"),
            xaxis=dict(
                domain=[0.0, 0.45],
                title="Buckling Severity Index ν (1 = low, 4 = severe)",
                range=[1.0, 4.1]
            ),
            xaxis2=dict(
                domain=[0.55, 1.0],
        ...
        fig3d_bsi.add_trace(go.Scatter3d(
            x=E[mask_valid], y=N[mask_valid], z=TVD[mask_valid],
            mode="markers",
            marker=dict(
                size=5,
        ...
        fig3d_bsi.update_layout(
            title="3D wellpath coloured by buckling severity",
            height=520,
            scene=dict(
                xaxis_title="East (ft)",
