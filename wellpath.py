...
# J / S / Horizontal synthetic survey + Minimum Curvature positions.
...
def _rf(dogs_rad: float) -> float:
    ...
    # RF = 2/dogs * tan(dogs/2); RF -> 1 as dogs -> 0
...
def _mcm_step(md1, inc1_deg, az1_deg, md2, inc2_deg, az2_deg,
    ...
    """One Minimum Curvature step  (industry-standard survey math).
       Returns (N2, E2, TVD2, DLS_deg_per_100ft)."""
    ds = md2 - md1
...
def _synthesize(profile: str,
    ...
    """Build a synthetic survey table (MD_ft, Inc_deg, Az_deg) for J / S / Horizontal."""
    ...
    while True:
        ...
        if after_kop:
            if profile == "Build & Hold" or profile.startswith("Horizontal"):
                ...
                if not built_to_theta:
                    nxt_inc = min(prev_inc + build_rate * (ds / 100.0), float(theta_hold_deg or 0.0))
                    built_to_theta = abs(nxt_inc - float(theta_hold_deg or 0.0)) < 1e-9
                    in_hold = (built_to_theta and (hold_left is not None) and (hold_left > 0.0))
                elif in_hold:
                    nxt_inc = float(theta_hold_deg or 0.0)
                    if hold_left is not None:
                        hold_left -= ds
                        if hold_left <= 0.0:
                            in_hold = False; in_drop = True
                elif in_drop:
                    nxt_inc = max(prev_inc - float(drop_rate or 0.0) * (ds / 100.0),
                                  float(final_inc_after_drop_deg or 0.0))
                    if abs(nxt_inc - float(final_inc_after_drop_deg or 0.0)) < 1e-9:
                        in_drop = False
                else:
...
def build_survey(profile: str,
                 ...
                 target_tvd_ft: Optional[float] = None,
    ...
    return _synthesize(profile, kop_md_ft, azimuth_deg, ds_ft, build_rate,
                       theta_hold_deg, target_md_ft, target_tvd_ft,
                       hold_length_ft, drop_rate, final_inc_after_drop_deg, lateral_length_ft)
